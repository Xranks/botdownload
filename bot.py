...
SERVER_ID = 2137242718


api_id = 2411982
api_hash = "80bb13b408a8a508711fd4605cfaadc1"
...
def mangolive(link):
        ...
        try:
                ...
                return str(nama)+' - '+str(shortId)+' - Mangolive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
        except:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(content[1])+' - Mangolive - '+str(waktu))
                return str(content[1])+' - Mangolive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
...
def ambilnamadream(idx):
        connection = psycopg2.connect(user="bzukhuhoiqaksy",
                                                                  ...
                                                                  database="drtesg2ah18ti")
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM host WHERE idhost = %s",[idx])
        connection.commit()
        tupples = cursor.fetchone()
        return tupples
...
def sugarlive(link):
        ...
        if namahost is not None:
                ...
                return str(namahost[2])+' - '+str(namahost[1])+' - Sugarlive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
        else:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(content[1])+' - Sugarlive - '+str(waktu))
                return str(content[1])+' - Sugarlive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'

def dreamlive(link):
        link = link
        if "dream_" in link:
                content = link.split('_')[1]
                namahost = ambilnama(content)
        else:
                content = link.split('_')[0].split('/')[4][1:]
                namahost = ambilnamadream(content)
        ...
        if namahost is not None:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(namahost[2])+' - '+str(namahost[1])+' - Dreamlive - '+str(waktu))
                return str(namahost[2])+' - '+str(namahost[1])+' - Dreamlive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
        else:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(content)+' - Dreamlive - '+str(waktu))
                return str(content)+' - Dreamlive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
...
def honeylive(link):
        ...
        try:
                ...
                return str(namaNickname)+' - '+str(content)+' - Honeylive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
        except:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(content)+' - Honeylive - '+str(waktu))
                return str(content)+' - Honeylive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
...
def cherrylive(link):
        ...
        API_ENDPOINT = "https://xhrky092ahclk.bokep00.com/App/Live/RecommendList?page=1"
        headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "X-Token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJuYmYiOjE2MjYzMTY3OTMuNjc2NTI5LCJpYXQiOjE2MjYzMTY3OTMuNjc2NTI5LCJleHAiOjE2MjY1NzU5OTMuNjc2NTI5LCJpZCI6IjI1MjI4Iiwic2hvd19pZCI6IjEwMDI5NTc3NjQiLCJzdGF0dXMiOiIxIiwidHlwZSI6IjIiLCJsZXZlbCI6IjEiLCJ2aXAiOjF9.nQfc0AYV0ZEjHNqtsvptv8DrvVAPXSWOjFHKSFj4fpY",
        ...
        x = True
        while x:
                API_ENDPOINT = "https://xhrky092ahclk.bokep00.com/App/Live/RecommendList?page="+str(y)
                r = requests.post(url = API_ENDPOINT, headers=headers, timeout=5)
                data_live_baru = r.json()
                if not data_live_baru['result']:
                        x = False
        ...
        try:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(namaNickname)+' - Cherrylive - '+str(waktu))
                return str(namaNickname)+' - '+str(content)+' - Cherrylive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
        except:
                with open("namahost.txt", "w", encoding="utf-8") as f:
                        f.write(str(content)+' - Cherrylive - '+str(waktu))
                return str(content)+' - Cherrylive - '+str(waktu)+' - Credit : @mangodansugarlivebotinfo'
...
def upload(update: Update, context: CallbackContext) -> None:
        ...
        if os.path.isfile('Credit @mangodansugarlivebotinfo.mp4'):
                print ("File exist")
                os.remove('Credit @mangodansugarlivebotinfo.mp4')
        ...
        if os.path.isfile('@mangolivedownload.flv'):
                ...
                subprocess.call(['ffmpeg', '-i', '@mangolivedownload.flv', '-codec', 'copy', 'Credit @mangodansugarlivebotinfo.mp4','-y'])
                try:
                        bot.deleteMessage(message_id =convertText['message_id'], chat_id =convertText['chat']['id'])
                except:
                        pass
                niceclient('Credit @mangodansugarlivebotinfo.mp4',content[0], update, context)
...
def echo(update: Update, context: CallbackContext) -> None:
        if  update.message.chat.id == akunUser or update.message.chat.id == SERVER_ID:
                ...
                if os.path.isfile('Credit @mangodansugarlivebotinfo.mp4.part'):
                        print ("File exist")
                        os.remove('Credit @mangodansugarlivebotinfo.mp4.part')
                ...
                findTexttttt=re.findall(r'(?<=dreamcast)', update.message.text)
                findTextttttt=re.findall(r'(?<=bokep222)', update.message.text)
                ...
                if findText:
                        ...
                        namaFile = cherrylive(update.message.text)
                        url=update.message.text
                        url=url.replace('.flv','')
                        print(namaFile)
                        print('Download Dari Cherrylive')
                elif findTexttttt:
                        namaFile = dreamlive(update.message.text)
                        url=update.message.text
                        url=url.replace('.flv','')
                        print(namaFile)
                        print('Download Dari Dreamlive')
                else:
                        url=update.message.text
                ...
                if findText or findTextt or findTexttt or findTextttt or findTexttttt or findTextttttt:
                        outtmpl = '@mangolivedownload.flv'
                        dbl = partial(my_hook,start=start,uploadText= uploadText,namaFile=str(namaFile))
                else:
                        outtmpl = 'Credit @mangodansugarlivebotinfo.mp4'
                ...
                if findTextttt or findTextttttt:
                        ...
                        try:
                                ...
                                pass
                        niceclient('Credit @mangodansugarlivebotinfo.mp4',namaFile, update, context)
                else:
                        try:
                                with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                                        ydl.download([str(url)])
                                convertText = update.message.reply_text('Sedang menconvert video')
                                subprocess.call(['ffmpeg', '-i', outtmpl, '-codec', 'copy', 'Credit @mangodansugarlivebotinfo.mp4','-y'])
                                try:
                                        bot.deleteMessage(message_id =convertText['message_id'], chat_id =convertText['chat']['id'])
                                except:
                                        pass
                                ...
                                try:
                                        ...
                                        urllib.request.urlretrieve(url, 'Credit @mangodansugarlivebotinfo.mp4')
                                        print ("Downloading :%s Selesai" % video_name)
                                        print(os.path.getsize('Credit @mangodansugarlivebotinfo.mp4'))
                                        niceclient('Credit @mangodansugarlivebotinfo.mp4','@mangolivedownload',update,context)
                                except:
                                        update.message.reply_text('Link Mati/Bot Tidak Support')
        else:
                update.message.reply_text('''
Bot ini punya orang lain hehe :)
kalau mau pakai bot bisa Order Bot DM: @LordRanggaSundaEmpire
...
lengkapnya bisa baca di : https://t.me/botrecordlive
Untuk Order DM admin : @LordRanggaSundaEmpire
                        ''')
        ...
        async def upload_file():
                ...
                try:
                        ...
                        try:
                                ...
                                print('berhasil')
                                await client.send_file(SERVER_ID, file, thumb=thumb, caption=namaFile, attributes=attributes, supports_streaming=True)
                                print('berhasil')
